...
function cheatsheet_glmakie()
    ...
    axs = [Axis3(fig[i,j], aspect = (1,1,1),
        ...
        for i in 3:7 for j in 1:5]
    [axs[i].titlegap = -15 for i in 1:25]
    #length(axs)
    ...
    linesegments!(axs[1],
        Point3f.(vec([[x, rand(), rand()] for i in 1:2, x = rand(10)]));
    ...
    ps = [Point3f(x, y, z) for x in -3:1:3 for y in -3:1:3 for z in -3:1:3]
    ...
    image!(ax0, rotr90(load("logoMakie.png")))
...
#fig = cheatsheet_glmakie()
#save("cheatsheet_glmakie.png", fig)